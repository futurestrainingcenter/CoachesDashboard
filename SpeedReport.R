...
generate_speed_report <- function(client_data, speed_data, athlete, month, year) { 
  ...
  if (nrow(vuemotion_fly_data) > 0) {
    ...
    posture <- image_crop(raw_fly_sfbgd, "525x475+200+575")
    report_img <- image_composite(report_img, posture, offset = "+1025+2600")
    rm(posture); gc()
    
    leg1 <- image_crop(raw_fly_sfbgd, "525x475+2325+575")
    report_img <- image_composite(report_img, leg1, offset = "+1600+2600")
    rm(leg1); gc()
    
    leg2 <- image_crop(raw_fly_sfbgd, "525x475+1590+575")
    report_img <- image_composite(report_img, leg2, offset = "+2125+2600")
    rm(leg2, raw_fly_sfbgd); gc()
    ...
    fly_fallback_path <- file.path(athlete_dir, paste0(athlete, " - fly_date.png"))
